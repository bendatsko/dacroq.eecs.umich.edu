...
-1 -3 -4 0`);
  ...
  useEffect(() => {
    // Use mock preset data - no external API needed
    setPresets([
      "Darpa_TNE_3SAT",
      "Darpa_TNE_2SAT", 
      "uf50-218_1-100",
  ...
  const handleSampleSelect = (sample: string) => {
    ...
    setTestName(sample);
    setPresetTotal(0);
    // Mock preset count data - no external API needed
    const mockTotal = Math.floor(Math.random() * 100) + 50; // Random number between 50-150
    setPresetTotal(mockTotal);
    setPresetLimit(mockTotal);
  };
  ...
  const logTestToDatabase = async (testId: string, testConfig: any) => {
    try {
      ...
      console.log("Test logged (mock):", testId, docData);
    } catch (error) {
  ...
  const handleRunTest = async () => {
    ...
    try {
      ...
      // Mock test submission - no external API needed
      console.log("Mock submitting test:", {
        testId,
        name: testName,
        inputType,
        ...(inputType === "preset" && {
