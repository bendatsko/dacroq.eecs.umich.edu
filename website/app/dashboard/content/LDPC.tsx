...
const RecentLdpcTests = ({ onViewDetails }: { onViewDetails: (test: TestRun) => void }) => {
  ...
  useEffect(() => {
    const fetchRecentTests = async () => {
      try {
        const testsQuery = query(collection(db, "tests"), orderBy("created", "desc"), limit(5));
        const querySnapshot = await getDocs(testsQuery);
        const testsData: TestRun[] = querySnapshot.docs
          .map((doc) => ({ id: doc.id, ...doc.data() }))
          .filter((test) => test.chipType === "LDPC") as TestRun[];
        setTests(testsData);
      } catch (error) {
...
const LDPCContent = ({ setActiveSection }: { setActiveSection: (section: any) => void }) => {
  ...
  useEffect(() => {
    const chipDocRef = doc(db, "chips", "ldpc");
