...
// Simplified Top Navigation Bar (No User Accounts)
const TopNavBar = ({
  theme,
  onNavClick,
  onThemeToggle,
      ...
      <div className="container flex h-16 max-w-screen-2xl items-center justify-between px-4 md:px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex flex-col">
            <h1 className="text-xl font-bold text-foreground tracking-tight">DACROQ</h1>
            <span className="hidden sm:block text-xs text-muted-foreground font-medium -mt-1">SAT Solver Platform</span>
          </div>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-1">
          <Button 
            ...
            onClick={() => onNavClick("3-SAT Solver")}
          >
          ...
          <Button 
            variant="ghost" 
            className="h-9 px-4 text-sm font-medium"
            onClick={() => onNavClick("LDPC Decoder")}
          >
          ...
          <Button 
            variant="ghost" 
            className="h-9 px-4 text-sm font-medium"
            onClick={() => onNavClick("k-SAT Solver")}
          >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button 
                  ...
                  onClick={() => onNavClick("Documentation")} 
                >
            ...
            <div className="md:hidden">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  <DropdownMenuItem onClick={() => onNavClick("3-SAT Solver")}>
                    <Cpu className="h-4 w-4 mr-2" />
                    3-SAT Solver
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onNavClick("LDPC Decoder")}>
                    <Activity className="h-4 w-4 mr-2" />
                    LDPC Decoder
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onNavClick("k-SAT Solver")}>
                    <BarChart3 className="h-4 w-4 mr-2" />
                    k-SAT Solver
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => onNavClick("Documentation")}>
                    <Book className="h-4 w-4 mr-2" />
                    Documentation
                  </DropdownMenuItem>
                </DropdownMenuContent>
...
const DashboardContent = ({ onNavClick }: { onNavClick: (title: string, parentTitle?: string) => void }) => {
  ...
  useEffect(() => {
    const fetchTests = async () => {
      try {
        // Mock test data - no Firebase needed
        const mockTests: TestRun[] = [
          {
            id: "test-1",
            name: "3-SAT Problem Test",
            chipType: "3-SAT",
            status: "completed",
            created: new Date(Date.now() - 1000 * 60 * 15).toISOString(), // 15 min ago
            runtime: "1.2s",
            result: "Satisfiable - Solution found"
          },
          {
            id: "test-2", 
            name: "LDPC Decode Test",
            chipType: "LDPC",
            status: "running",
            created: new Date(Date.now() - 1000 * 60 * 5).toISOString(), // 5 min ago
            runtime: "2.8s",
            result: "In progress..."
          },
          {
            id: "test-3",
            name: "k-SAT Solver Test",
            chipType: "k-SAT", 
            status: "completed",
          ...
          {
            id: "test-4",
            name: "Large 3-SAT Instance",
            chipType: "3-SAT",
            status: "queued",
            created: new Date(Date.now() - 1000 * 30).toISOString(), // 30 sec ago
            runtime: "-",
            result: "Waiting in queue..."
          },
          {
            id: "test-5",
            name: "LDPC Performance Test",
            chipType: "LDPC", 
            status: "completed",
            created: new Date(Date.now() - 1000 * 60 * 120).toISOString(), // 2 hours ago
            runtime: "4.1s",
            result: "98.7% decoding rate"
          }
        ];
        setTests(mockTests);
      } catch (error) {
  ...
  const handleBulkDelete = () => {
    ...
    if (confirm(confirmMessage)) {
      // Mock delete operation - no Firebase needed
      try {
        console.log("Mock deleting tests:", Array.from(selectedTests));
  ...
  const handleSaveRename = async (testId: string, newName: string) => {
    try {
      // Mock update operation - no Firebase needed
      console.log("Mock renaming test:", testId, "to", newName);
      setTests((prev) =>
  ...
  const handleDeleteTest = async (test: TestRun) => {
    try {
      ...
      // Mock delete operation - no Firebase needed
      console.log("Mock deleting test:", test.id);
...
// --------------------
// Main Dashboard Page Component (Simplified, No Authentication)
// --------------------
