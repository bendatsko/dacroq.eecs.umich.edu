...
export function UserManagementModal({ open, onOpenChange }: UserManagementModalProps) {
  ...
  // Initialize with static data (no Firebase)
  useEffect(() => {
    const initializeUsers = () => {
      ...
      // Use static data from ALLOWED_USERS
      const initialUsers = ALLOWED_USERS.map(user => ({
        ...user,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        status: 'active' as const,
        testsRun: Math.floor(Math.random() * 50), // Mock data
      ...
      setSystemStats({
        ...
        testsRunToday: Math.floor(Math.random() * 20),
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      setSystemStats(prev => ({ ...prev, activeUsers: prev.activeUsers + 1 }));
      setNewEmail('');
      setNewIsAdmin(false);
  ...
  const handleRemoveUser = async () => {
    ...
    try {
      ...
      setSystemStats(prev => ({ ...prev, activeUsers: Math.max(0, prev.activeUsers - 1) }));
  ...
  const handleToggleAdmin = async (email: string, currentIsAdmin: boolean) => {
    try {
      setUsers(users.map(user =>
          user.email === email ? { ...user, isAdmin: !currentIsAdmin, updatedAt: new Date().toISOString() } : user
      ));
  ...
  const handleToggleStatus = async (email: string, currentStatus: string) => {
    try {
      ...
      setUsers(users.map(user =>
          user.email === email ? { ...user, status: newStatus as 'active' | 'inactive', updatedAt: new Date().toISOString() } : user
      ));
      
      // Update active user count
      const activeCount = users.filter(u => 
        u.email === email ? newStatus === 'active' : u.status === 'active'
      ).length;
      setSystemStats(prev => ({ ...prev, activeUsers: activeCount }));
